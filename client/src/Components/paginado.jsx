...
function Paginado({ sneakersPerPage, allSneackers, currentPage, setCurrentPage }) {
  ...
  useEffect(() => {
    let paginas = [1];
    for (let i = 2; i <= Math.ceil(allSneackers / sneakersPerPage) + 1; i++) {
      paginas.push(i);
    }
    setPages(paginas);
  }, [allSneackers, sneakersPerPage]);
